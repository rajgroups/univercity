                        ...
                        <div class="course-card">
                            <a href="{{ route('web.global.course', ['countries' => $item->iso3]) }}" class="card-img">
                                @if(isset($item->iso2) && $item->iso2)
                                  ...
                                  <img
    src="https://flagcdn.com/w320/{{ strtolower($item->iso2 ?? 'xx') }}.png"
    alt="{{ $item->name }} flag"
    onerror="this.onerror=null; this.src='https://flagcdn.com/w320/xx.png';">

                                @elseif(isset($item->emoji) && $item->emoji)
                                    {{-- Fallback to emoji --}}
                                    <div class="flag-placeholder" style="height: 180px; display: flex; align-items: center; justify-content: center; font-size: 48px; background: #f5f5f5;">
                                        {{ $item->emoji }}
                                    </div>
                                @else
                                    {{-- Default fallback --}}
                                    <img src="{{ asset('default-flag.png') }}" alt="Default flag">
                                @endif
                            </a>
