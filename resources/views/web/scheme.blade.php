...
@push('meta')
    ...
    <meta name="description"
        content="{{ $metaDescription ?? Str::limit(strip_tags($announcement->description ?? ''), 160) }}">
...
@section('content')
    <style>
        ...
        .sticky-nav-card {
            ...
            scrollbar-color: var(--scheme-primary) #f1f1f1;
        }
    ...
    <section class="scheme-hero">
        <div class="scheme-hero-bg"
            style="background-image: url({{ asset($announcement->banner_image ?? ($announcement->image ?? 'assets/images/placeholder.jpg')) }})">
        </div>
        ...
        <div class="container-fluid px-3 px-md-4 px-lg-5">
            <div class="row align-items-center">
                <div class="col-lg-8">
                    ...
                    <p class="scheme-subtitle-modern text-white">
                        {{ $announcement->subtitle ?? 'Skill development and empowerment initiative by the government.' }}
                    </p>
            ...
            <div class="col-lg-4">
                <div class="sticky-nav-card shadow-sm border-0">
                    ...
                    <div class="mt-5 p-4 bg-light rounded-4">
                        ...
                        <p class="small text-muted mb-4">Contact our support desk for registration help and more details.
                        </p>
                        <a href="{{ route('contact') }}" class="btn btn-primary w-100 rounded-pill fw-bold py-2">Contact
                            Us</a>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-5 pt-5 border-top">
                <div class="d-flex justify-content-between align-items-end mb-4">
                    <div>
                        ...
                        <h2 class="scheme-heading-section mb-0">Similar Government Schemes</h2>
                    </div>
                    <!-- Navigation Buttons -->
                    <div class="d-none d-md-flex gap-2">
                        <button class="scheme-swiper-prev btn btn-light rounded-circle shadow-sm"
                            style="width: 45px; height: 45px;">
                            <i class="bi bi-chevron-left"></i>
                        </button>
                        <button class="scheme-swiper-next btn btn-primary rounded-circle shadow-sm"
                            style="width: 45px; height: 45px;">
                ...
                <div class="swiper similarSchemesSwiper">
                    <div class="swiper-wrapper py-3">
                        @foreach ($similars as $similar)
                            <div class="swiper-slide h-auto">
                                <div
                                    class="similar-card h-100 d-flex flex-column border-0 shadow-sm hover-elevate transition-all bg-white rounded-4 overflow-hidden">
                                    <div class="similar-img-wrapper position-relative" style="height: 200px;">
                                        <img src="{{ asset($similar->image ?? 'assets/images/placeholder.jpg') }}"
                                            class="w-100 h-100 object-fit-cover transition-transform"
                                            alt="{{ $similar->title ?? 'Scheme' }}">
                                        <div class="position-absolute top-0 end-0 m-3">
                                            <span
                                                class="badge bg-white text-primary rounded-pill px-3 py-2 shadow-sm small fw-bold">
                                                {{ $similar->category?->name ?? 'Common' }}
                                            </span>
                                        </div>
                                    </div>
                                    <div class="p-4 d-flex flex-column flex-grow-1">
                                        <h5 class="fw-bold mb-3 text-dark line-clamp-2" style="min-height: 3rem;">
                                            {{ $similar->title ?? 'Untitled Scheme' }}
                                        </h5>
                                        <p class="text-muted small mb-4 line-clamp-2 flex-grow-1">
                                            {{ Str::limit(strip_tags($similar->description ?? ''), 90) }}
                                        </p>
                                        <div class="mt-auto pt-3 border-top border-light">
                                            <a href="{{ route('web.announcement.scheme', [$similar->category?->slug ?? 'common', $similar->slug]) }}"
                                                class="btn btn-outline-primary w-100 rounded-pill fw-bold text-uppercase"
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Check if content height is greater than max-height (250px)
            if (desc && desc.scrollHeight > 250) {
        ...
        function toggleDescription() {
            ...
            if (desc.classList.contains('collapsed')) {
                ...
                // Optional: scroll to top of overview
                // document.getElementById('overview').scrollIntoView({ behavior: 'smooth', block: 'start' });
            }
        }

        // Initialize Similar Schemes Swiper
        var similarSchemesSwiper = new Swiper(".similarSchemesSwiper", {
            slidesPerView: 1,
            spaceBetween: 24,
            loop: false,
            grabCursor: true,
    ...
    <style>
        ...
        .hover-elevate:hover .similar-img {
