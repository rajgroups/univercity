                                ...
                                <div class="col-sm-6 col-12">
                                    <div class="mb-3">
                                        ...
                                        <input type="file"
                                            ...
                                            name="banner_images" accept="image/*" required>
                                        @error('banner_images')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select select2" name="stakeholders[]" multiple>
                                            @foreach($dbStakeholders as $st)
                                                <option value="{{ $st->id }}" {{ in_array($st->id, old('stakeholders', [])) ? 'selected' : '' }}>
        ...
        $(document).ready(function() {
            ...
            @if($errors->any())
                ...
                @foreach($errors->keys() as $key)
                    // Convert array keys to field names (e.g. data.0 to data[0])
                    @php $fieldName = str_replace('.', '[', $key) . (strpos($key, '.') !== false ? ']' : ''); @endphp
                    var field = document.querySelector('[name="{{ $fieldName }}"]') || document.querySelector('[name="{{ $key }}"]');
                    if (field) {
                        field.classList.add('is-invalid');
                        var feedback = field.nextElementSibling;
                        if(feedback && feedback.classList.contains('invalid-feedback')) {
                            feedback.innerHTML = "{{ $errors->first($key) }}";
                            feedback.classList.add('d-block');
                        } else {
                            field.insertAdjacentHTML('afterend', '<div class="invalid-feedback html5-invalid-feedback d-block">{{ $errors->first($key) }}</div>');
                        }
